...
use lisp::defsubr;
use remacs_sys::{globals, make_lisp_ptr};
...
fn cons(car: LispObject, cdr: LispObject) -> LispObject {
    ...
    unsafe {
        if !cons_free_list.is_null() {
            ...
            val = LispCons(make_lisp_ptr(cons_free_list as *mut libc::c_void, Lisp_Type::Lisp_Cons));
            let new_list_head = (*cons_free_list).u.next;
